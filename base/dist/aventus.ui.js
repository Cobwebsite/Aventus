...
let Date=class Date {
    ...
    endOfWeek(firstDay = DateSettings.current.firstDayOfWeek) { return this.startOfWeek(firstDay).addDays(6); }
    diffDays(other) { return Math.abs(this.value.until(other.value, { largestUnit: 'days' }).days); }
    toLocaleString(options, locale) {
        const config = DateSettings.current;
...
this.__addPropertyActions("mode", ((target) => {
    ...
    static __style = `:host{--_img-color: var(--img-color);--_img-stroke-color: var(--img-stroke-color, var(--_img-color));--_img-fill-color: var(--img-fill-color, var(--_img-color));--_img-color-transition: var(--img-color-transition, none)}:host{display:inline-block;overflow:hidden;font-size:0}:host *{box-sizing:border-box}:host img{opacity:0;transition:filter .3s linear}:host .svg{display:none;height:100%;width:100%}:host .svg svg{height:100%;width:100%}:host([src$=".svg"]) img{display:none}:host([src$=".svg"]) .svg{display:flex}:host([src$=".svg"]) .svg svg{transition:var(--_img-color-transition);stroke:var(--_img-stroke-color);fill:var(--_img-fill-color)}:host([display_bigger]) img{cursor:pointer}:host([display_bigger]) img:hover{filter:brightness(50%)}`;
    __getStatic() {
        return Img;
    }
    __getStyle() {
...
Navigation.Router = class Router extends Aventus.WebComponent {
    ...
    this.__getStatic().__template.setHTML({
        ...
        blocks: { 'default':`<slot name="before"></slot><main class="content" _id="router_0"></main><slot name="after"></slot>` }
    });
}
    __registerTemplateAction() { super.__registerTemplateAction();
this.__getStatic().__template.setActions({
